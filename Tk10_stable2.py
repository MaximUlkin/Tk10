...
def delStudent():
	...
	def delproceed():
		...
		flag = True
		for i in range(Student.studentCount):
			if fname_e.get() == s[i].first and lname_e.get() == s[i].last:
				for x in range(i+1,Student.studentCount):
...
def delallStudent():
	...
	def delallsuccess():
		...
		,height=1,width=6,fg="green",command=finalbtn)
...
s.append(Student("Kevin","Hart",17,12,"Literature"))
s.append(Student("Bob","Jefferson",16,11,"Maths"))
